...
## Data files (hosted on OSF)

Due to repository size limits, the following large files are hosted externally:

- `virality.db`
- `contextual_features.json`

You can download them from OSF here:

```text
https://osf.io/dqncu/files/osfstorage
```

After downloading, place `virality.db` under `DATA_DIR` (see **Expected data layout**). If you use `contextual_features.json`, place it where the notebook expects it (search for `contextual_features.json` in the notebook to confirm the path).
...
~/Reddit_Virality_Data/
  virality.db
  media_files/
    <post_id>.jpg|png|jpeg|gif|webp
```

If you plan to run deep visual baselines, make sure `media_files/` is populated (see the section below).

To change this, edit `DATA_DIR` (search for `DATA_DIR =` in the notebook).
...
**Note:** Depending on network speed and how many URLs are still reachable, downloading the full media set can take **many hours and potentially up to a day**. This step is **only required** if you want to run the **deep baseline comparisons**. If you are only running classical baselines / feature-based experiments, you can **skip** media downloading and ignore the deep-baseline cells.
